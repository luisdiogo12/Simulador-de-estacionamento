...
class Car {
  ...
  constructor(keyMap, world) {
    ...
    const suspensionRestLength = 1;
    console.log(
      "this.wheelFL.mesh.position.y - wheelRadius: ",
      this.wheelFL.mesh.position.y - wheelRadius
    );
    /* [this.wheelFL, this.wheelFR, this.wheelBL, this.wheelBR].forEach((w) => {
      const chassisConnectionCs = w.mesh.position;
      const suspensionRestLength = 1000; //w.mesh.position.y; //ntrolador apenas lança um raio de 2 unidades para baixo e nunca chega ao chão que está 6 u abaixo do ponto de origem
      const directionCs = new RAPIER.Vector3(0, -1, 0);
      const axleCs = new RAPIER.Vector3(-1, 0, 0);
      console.log(
        `Car: ${
          w.mesh.name
        } chassisConnectionCs: ${chassisConnectionCs.toArray()} suspensionRestLength: ${suspensionRestLength} `
      );
      this.vehicleController.addWheel(
        chassisConnectionCs, // chassisConnectionCs: Vector  - espaço local do chassis
        directionCs, // directionCs: Vector          - Se o chassis está orientado com Y para cima, use (0, -1, 0)
        axleCs, // axleCs: Vector               - Se a roda está rotacionada 90 graus em torno do eixo Z, o eixo de rotação é X
